...
class CourseCohortService {
    ...
    static async updateCourseCohort(courseCohortId: number, updates: unknown) {
        ...
        return timetablingAxiosInstance.patch(`/course-cohorts/${courseCohortId}/trainer`, updates)
            .catch(err => console.log('error occurred while assigning a trainer to course cohort', err));
    }
}
