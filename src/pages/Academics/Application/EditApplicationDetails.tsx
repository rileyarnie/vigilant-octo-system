...
export function EditApplicationDetails (props) {
  ...
  const [firstName, setFirstName] = useState(
      props.application?.applications_firstName || ''
  );
  const [lastName, setLastName] = useState(
      props.application?.applications_lastName || ''
  );
  const [otherName, setOtherName] = useState(
      props.application?.applications_otherName || ''
  );
  const [nationality, setNationality] = useState(
      props.application?.applications_nationality || ''
  );
  const [identification, setIdentification] = useState(
      props.application?.applications_identification || ''
  );
  const [identificationType, setIdentificationType] = useState(
      props.application?.applications_identification || ''
  );
  const [emailAddress, setEmailAddress] = useState(
      props.application?.applications_emailAddress || ''
  );
  const [gender, setGender] = useState(props.application?.applications_gender || '');
  const [maritalStatus, setMaritalStatus] = useState(
      props.application?.applications_maritalStatus || ''
  );
  const [religion, setReligion] = useState(
      props.application?.applications_religion || ''
  );
  const [dateOfBirth, setDateOfBirth] = useState(
      props.application?.applications_dateOfBirth || ''
  );
  const [phoneNumber, setPhoneNumber] = useState(
      props.application?.applications_phoneNumber || ''
  );
  const [fileUploaded, setFileUploaded] = useState(
      props.application?.applications_otherName
  );
  const [nextOfKinName, setNextOfKinName] = useState(
      props.application?.nkd_name || ''
  );
  const [nextOfKinPhoneNumber, setNextOfKinPhoneNumber] = useState(
      props.application?.nkd_nextOfKinPhoneNumber || ''
  );
  const [nextOfKinRelation, setNextOfKinRelation] = useState(
      props.application?.nkd_relation || ''
  );
  const [physicalChallenges, setPhysicalChallenges] = useState(
      props.application?.applications_physicalChallenges || ''
  );
  const [courseStartDate, setCourseStartDate] = useState(
      props.application?.applications_courseStartDate || ''
  );
  const [campus, setCampus] = useState(props.application?.applications_campus);
  const [sponsor, setSponsor] = useState(
      props.application?.applications_sponsor || ''
  );
  const [countyOfResidence, setCountyOfResidence] = useState(
      props.application?.applications_countyOfResidence || ''
  );
  const [documentsUrl, setDocumentsUrl] = useState(
      props.application?.applications_documentsUrl || ''
  );
  const [campuses, setCampuses] = useState([]);
  const [showUploadModal, setShowUploadModal] = useState(false);
  JSON.parse(
        localStorage.getItem('programId') as string
  );
